...
void spline_vdv(double rmin, double rmax, 
		...
		double alp, double qij, int iperd,int itype, int int_res_new,int ishift,
...
/* II) get positions at which to evaluate potential and force               */
  ...
  *dri = 1.0/(*dr);
  ...
  if(ishift==2){
    /* hard-wire 1 angstrom as healing length (since respa length is longer
       in this new inter formulation) */ 
    /* rheal=1.88972 */
    for (i = 1; i <= nsplin; ++i) {
  ...
  switch(itype){
   /* Coulomb */
    case 1: vcoul_bin(nsplin,r,cv0,cdv0,sw,dsw,qij,iperd,alp,diele_opt,
                      diele_rheal,diele_cut,diele_eps,itype,int_res_new); break; 
   /* LJ      */
   ...
   /* for new respa split of the ewald erf/r */
    case 7: vcoul_bin(nsplin,r,cv0,cdv0,sw,dsw,qij,iperd,alp,diele_opt,
                      diele_rheal,diele_cut,diele_eps,itype,int_res_new); break;
  }/*end switch*/
/*==========================================================================*/
...
void vcoul_bin(int n,double r[],double v[],double dv[],
               ...
               double diele_rheal, double diele_cut,double diele_eps,int itype,int int_res_new)
...
/* Real space ewald */
  ...
  if (iperd > 0 && int_res_new==0) {
    ...
    /* new respa split */
  } else if(iperd > 0 && int_res_new==1) {
    if(itype==1) {
      talp2 = 2.0*alp*alp;
      palp  = p*alp;
    /* unscreeened coulomb - short range */
      for(i=1;i<=n;i++){
        /* vnow  = qij/r[i]; */
        /* ok, we are cheating here, the potential is not used with RESPA
           (since the integration only requires the forces), so we'll just
           keep the usual erfc/r in the potential, so that no modifications need
           to be made in the potential computation down the line */
        r2     = r[i]*r[i];
        ralp   = r[i] * alp;
        eee    = exp(-ralp*ralp);
        tt     = 1.0/(1.0+p*ralp);
        ...
        dgerfc = ((((((((de9*tt+de8)*tt+de7)*tt+de6)*tt+de5)*tt
                             ...
                             +talp2*gerfc*r[i];
        vnow2   = qij * gerfc/r[i];
        /* but the force is the unscreened coulomb as used in the respa inner loop */ 
        vnow  = qij/r[i];
        dvnow = qij/(r[i]*r[i]*r[i]);
      /*  v[i]  = vnow2*sw[i];
        dv[i] = dvnow*sw[i] + vnow*dsw[i]; */
    ...
    /* erf/x - long range */
      ...
      for(i=1;i<=n;i++){
        ...
        dgerfc = ((((((((de9*tt+de8)*tt+de7)*tt+de6)*tt+de5)*tt
                             +de4)*tt+de3)*tt+de2)*tt+de1)*tt*tt*eee*palp
                             +talp2*gerfc*r[i];
        vnow   = qij * (gerfc-1.0)/r[i];
        dvnow  = ((gerfc-1.0)/r2 + dgerfc/r[i])*qij/r[i];
        /* v[i]   = vnow*sw[i];
        dv[i]  = dvnow*sw[i] + vnow*dsw[i]; */
        v[i] = vnow;
        dv[i] = dvnow;
