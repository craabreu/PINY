...
/*========================================================================*/
  /*  8)\cp_dft_typ{lsda,lda,gga_lda,gga_lsda} */
   ...
   if(ifound != 1) keyarg_barf(dict,filename_parse->input_name,fun_key,index);
  /*-----------------------------------------------------------------------*/ 
  /* 1)\cp_vxc_typ{pz_lda,pw_lda,pz_lsda,pade_lda,pade_lsda}               */
  ...
  /* 23)\gradient_cutoff{#}   */
       ...
       if(cp->cpopts.cp_lsda == 1) 
          cp->pseudo.gga_cut *= 0.5; /*since n_up,n_dn = 0.5*n */
...
/*========================================================================*/
  ...
  /*-----------------------------------------------------------------------*/
  /* 34)\respa_inter_new{on,off} */
      ifound = 0;
      if(strcasecmp(dict[34].keyarg,"on")==0) {
      class->energy_ctrl.int_res_new=1;
      general_data->timeinfo.int_res_new=1;
      printf("\n");
      printf("===========================\n");
      printf("Using NEW inter respa split\n");
      printf("---------------------------\n");
      ifound++;}
      if(strcasecmp(dict[34].keyarg,"off")==0) {
      class->energy_ctrl.int_res_new=0;
      general_data->timeinfo.int_res_new=0;
      ifound++;
      }
      index=34;
    if(ifound != 1) keyarg_barf(dict,filename_parse->input_name,fun_key,index);   
  ...
  /* 8)\respa_rheal{#} */
       ...
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);
       if(class->interact.rheal_res>4.5){
       printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");    
       printf("You have requested RESPA healing length > 4.5A   \n");
       printf("Are you certain this is what you would like to do?\n");
       printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
       }/*endif*/
  ...
  /*-----------------------------------------------------------------------*/ 
  /* 35)\poten_rheal{#} */
      sscanf(dict[35].keyarg,"%lg",&(class->interact.rheal));
       index=35;
       if(class->interact.rheal<=0)
       keyarg_barf(dict,filename_parse->input_name,fun_key,index);
       if(class->interact.rheal>2.5){
       printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
       printf("You have requested POTENTIAL healing length > 2.5A   \n");
       printf("Are you certain this is what you would like to do?\n");
       printf("$$$$$$$$$$$$$$$$$$$$_warning_$$$$$$$$$$$$$$$$$$$$\n");
       }/*endif*/
       class->interact.rheal  /= BOHR;
  /*-----------------------------------------------------------------------*/ 
  /* 36)\respa_switch_order{3,5} */
      sscanf(dict[36].keyarg,"%lg",&(real_key_arg));
      class->interact.switord=(int)(real_key_arg);
      index=36;
      if(!(class->interact.switord==3 || class->interact.switord==5))
      keyarg_barf(dict,filename_parse->input_name,fun_key,index);
  /*-----------------------------------------------------------------------*/
  /* 9)\shake_tol{#} */
