...
void control_inter_params(INTERACT *interact,SPLINE_PARSE *spline_parse,
                          ...
                          int int_res_ter,int int_res_new,int myid, MPI_Comm comm,
                          int num_proc )
...
/* VI) Allocate spline arrays                                           */
  ...
  *tot_memory += now_mem;
  ...
  if(int_res_new==1) {
    interact->cv1_c  = (double *) cmalloc(nsplin_mall_tot*sizeof(double))-1;
    interact->cdv1_c = (double *) cmalloc(nsplin_mall_tot*sizeof(double))-1;
  }
...
/*     energy and forces                                                 */
 ...
 if(myid==0) {
    printf("\n-------------------------------------------\n");
    printf("Healing lengths for switching functions:\n");
    printf("Potential: %g angstrom\n",interact->rheal*0.52917721);
    printf("RESPA:     %g angstrom\n",interact->rheal_res*0.52917721);
    printf("-----------------------------------------\n\n");
 }
...
/* Spline the intermolecular interaction */
 ...
 set_inter_splin(sig,eps,awill,bwill,cwill,rm_swit,c6m,c8m,c9m,c10m,
                  ...
                  ninter_unique,ncharge,iperd,int_res_new,myid);
...
void set_inter_splin(double sig[],double eps[],double a[],double b[],
                     ...
                     int ninter,int ncharge,int iperd,int int_res_new,
                     int myid)
...
/*  I) Spline each interaction                                          */
  ...
  for(i=1;i <= ninter;i++) {
    ...
    spline_vdv(interact->rmin_spl[i],rmax,&(interact->cv0)[ioff],
	       ...
	       c6[i],c8[i],c9[i],c10[i],alp_ewd,qijp,iiperd,itype,int_res_new,ishift,
