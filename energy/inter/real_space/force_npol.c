...
/*             Store the Nasty Pointers for the dumb compiler             */
  ...
  int energy_ctrl_int_res_new     = energy_ctrl->int_res_new; 
  ...
  double *interact_cdv1_c         = interact->cdv1_c; /* for int_res_new==1 */
  ...
  if(energy_ctrl_iget_res_inter == 1) {
    for (jpart = 1; jpart <= intact_now; jpart++) {
      ...
      if(switord==3) {
        ...
        /* quintic spline */
        intra_scr_swit[jpart] = br*br*br*(-6.0*br*br+15.0*br-10.0) + 1.0;
      }
    }/*endfor*/
  }/*endif*/
...
void npol_force_calc(int intact_now,
		     double *intra_scr_del_r,
		     double *intra_scr_spl_tmp,
		     int *for_scr_i_indext,
		     double *interact_cdv0,
		     double *intra_scr_swit_hard,
		     double *intra_scr_fx1,
		     double *intra_scr_dx12,
		     double *intra_scr_fy1,
		     double *intra_scr_dy12,
		     double *intra_scr_fz1,
		     double *intra_scr_dz12,
		     double *intra_scr_fx2,
		     double *intra_scr_fy2,
		     double *intra_scr_fz2,
		     double *intra_scr_fx3,
		     double *intra_scr_fy3,
		     double *intra_scr_fz3,
		     double *intra_scr_q2,
		     double *intra_scr_r,
		     double *interact_cdv0_c,
		     double *interact_cdv1_c,
		     double *intra_scr_spl_tmp2,
		     double *intra_scr_spl_tmp3,
...
/*            Local variable declarations                                */
    ...
    int jpart,iii;
    double fmult,dvtemp,dvtemp2,intra_scr_r3;
    double flx,fly,flz,fsx,fsy,fsz;
      ...
      if(energy_ctrl_int_res_new==0) {
        ...
        for (jpart = 1; jpart <= intact_now; jpart++) {
         ...
         if((energy_ctrl_iget_full_inter==0) && (energy_ctrl_iget_res_inter==1)) {
          ...
          } else if((energy_ctrl_iget_full_inter==1) && (energy_ctrl_iget_res_inter==1)) { 
            /* outer loop of inter respa */
            npol_vspl_fetch(intact_now,intra_scr_del_r,intra_scr_spl_tmp2,
                  for_scr_i_indext,interact_cdv0_c,intra_scr_swit_hard);
            npol_vspl_fetch(intact_now,intra_scr_del_r,intra_scr_spl_tmp3,
                  for_scr_i_indext,interact_cdv1_c,intra_scr_swit_hard);
            for (jpart = 1; jpart <= intact_now; jpart++) {
              ...
              intra_scr_fz1[jpart] = intra_scr_dz12[jpart]*dvtemp;
              /* the long range part */
              dvtemp2 = intra_scr_q2[jpart]*intra_scr_spl_tmp3[jpart]; 
              intra_scr_fx3[jpart] = intra_scr_dx12[jpart]*dvtemp2;
              intra_scr_fy3[jpart] = intra_scr_dy12[jpart]*dvtemp2;
              intra_scr_fz3[jpart] = intra_scr_dz12[jpart]*dvtemp2;
            } /* endfor */
           } else if((energy_ctrl_iget_full_inter==1) && (energy_ctrl_iget_res_inter==0)) {
            /* no just one force for the whole inter_respa */
            /* do nothing this option with int_res_new is not allowed */
            printf("Problem with forces for int_res_new==1 and no inter respa\n");
          } 
       } /* end if (respa type) */
    }/*endif*/
}/*endif:coulomb interactions*/
...
/* III) RESPA forces                                                      */
  ...
  if((energy_ctrl_iget_full_inter==1)&&(energy_ctrl_iget_res_inter==1)){
    if(energy_ctrl_int_res_new==1) {
      for (jpart = 1; jpart <= intact_now; jpart++) {
	/* fmult  = (intra_scr_swit[jpart]*wght_res  
		 +  (1.0-intra_scr_swit[jpart])*wght_full); */
        fsx = intra_scr_swit[jpart]*intra_scr_fx1[jpart];
	fsy = intra_scr_swit[jpart]*intra_scr_fy1[jpart];
        fsz = intra_scr_swit[jpart]*intra_scr_fz1[jpart];

        flx = (1.0-intra_scr_swit[jpart])*intra_scr_fx1[jpart]+intra_scr_fx3[jpart];
        fly = (1.0-intra_scr_swit[jpart])*intra_scr_fy1[jpart]+intra_scr_fy3[jpart];
        flz = (1.0-intra_scr_swit[jpart])*intra_scr_fz1[jpart]+intra_scr_fz3[jpart];
/*
        if(intra_scr_r[jpart]>=9.5/0.529) {
          printf("r=%g\n",intra_scr_r[jpart]*0.529);  
          printf("fl= %g %g %g\n",flx,fly,flz);
          printf("fs= %g %g %g\n",fsx,fsy,fsz);
        }
*/
        intra_scr_fx2[jpart] = fsx*wght_res+flx*wght_full;  	
        intra_scr_fy2[jpart] = fsy*wght_res+fly*wght_full;
        intra_scr_fz2[jpart] = fsz*wght_res+flz*wght_full;

        /* intra_scr_fx2[jpart] = intra_scr_fx1[jpart]*fmult 
	  + intra_scr_fx3[jpart]*wght_full;
        	intra_scr_fy2[jpart] = intra_scr_fy1[jpart]*fmult
	  + intra_scr_fy3[jpart]*wght_full;
	intra_scr_fz2[jpart] = intra_scr_fz1[jpart]*fmult
	  + intra_scr_fz3[jpart]*wght_full; */
      /* for the benefit of the pressure tensor calc.
         store f1+f3 in f1 */
        intra_scr_fx1[jpart] += intra_scr_fx3[jpart];
        intra_scr_fy1[jpart] += intra_scr_fy3[jpart];
        intra_scr_fz1[jpart] += intra_scr_fz3[jpart];
      }/*endfor*/
    } else if(energy_ctrl_int_res_new==0) {
      for (jpart = 1; jpart <= intact_now; jpart++) {
	fmult = (intra_scr_swit[jpart]*wght_res  
		 +  (1.0-intra_scr_swit[jpart])*wght_full);
	intra_scr_fx2[jpart] = intra_scr_fx1[jpart]*fmult;
	...
	intra_scr_fz2[jpart] = intra_scr_fz1[jpart]*fmult;
/* 
       flx=(1.0-intra_scr_swit[jpart])*intra_scr_fx1[jpart];
        fly=(1.0-intra_scr_swit[jpart])*intra_scr_fy1[jpart];
        flz=(1.0-intra_scr_swit[jpart])*intra_scr_fz1[jpart];
       if(intra_scr_r[jpart]>=9.5/0.529) {
          printf("r=%g\n",intra_scr_r[jpart]*0.529);
          printf("fl= %g %g %g\n",flx,fly,flz);
        }
