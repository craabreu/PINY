...
/*======================================================================*/
/* II.V) Get the coloumb correction due to CP-CP and CP-Classical atoms */
...
/*  NOTE: THIS HAS NOT BEEN IMPLEMENTED FOR PATH INTEGRALS              */
  ...
  if( bonded->excl.num_cp > 0 && pi_beads == 1){
     mix_coul_corr(&(class->clatoms_info),(class->clatoms_pos),
                   &(bonded->intra_scr),&(bonded->excl),
                   &(class->interact),
                   &(general_data->cell),&(general_data->ptens),
                   &vcoul,general_data->ewald.alp_ewd);
  }/*endif*/
...
/*==========================================================================*/
  ...
  double *clatoms_x    = clatoms_pos[1].x;
  double *clatoms_y    = clatoms_pos[1].y;
  double *clatoms_z    = clatoms_pos[1].z;

  double *clatoms_fx   = clatoms_pos[1].fx;
  double *clatoms_fy   = clatoms_pos[1].fy;
  double *clatoms_fz   = clatoms_pos[1].fz;
  ...
  if (iperd == 0) {
   ...
   if(diele_opt==0){
    for(i=1;i<= num_cp;i++){
      ...
      dx12  = clatoms_x[(j1_cp[i])] - clatoms_x[(j2_cp[i])];
      dy12  = clatoms_y[(j1_cp[i])] - clatoms_y[(j2_cp[i])];
      dz12  = clatoms_z[(j1_cp[i])] - clatoms_z[(j2_cp[i])];
       
      r2    = dx12*dx12 + dy12*dy12 + dz12*dz12;
      r     = sqrt(r2);
      qij   = clatoms_q[(j1_cp[i])]*clatoms_q[(j2_cp[i])];
      ...
      *vcoul += v;
      ...
      /* Particle i */
      clatoms_fx[(j1_cp[i])] += fx;
      clatoms_fy[(j1_cp[i])] += fy;
      clatoms_fz[(j1_cp[i])] += fz;


      /* Particle j */
      clatoms_fx[(j2_cp[i])] -= fx;
      clatoms_fy[(j2_cp[i])] -= fy;
      clatoms_fz[(j2_cp[i])] -= fz;
