...
void get_clong(int natm_tot,int natm_typ,int ninter,
	       ...
	       double cutoff_res[],int iswit, double rheal, double rheal_res)
  ...
  if(iswit==0){
   for(i=1;i<=ninter;i++){
     ...
     *clong_res -= fpi*c6m[inter_map_index_i]*clongm[i]*(
                   (1.0/(rheal_res*rheal_res))*(1.0/rmin+1.0/(rmin-rheal_res))
                  +(2.0/(rheal_res*rheal_res*rheal_res))*log((rmin-rheal_res)/rmin));
   }/*endfor*/
  }else{
   for(i=1;i<=ninter;i++){
     inter_map_index_i = inter_map_index[i];
      ...
      *clong     -= (fpi*c6m[inter_map_index_i]*clongm[i]*fact);
      fact_res    = (1.0/(rheal_res*rheal_res))*(1.0/rmin+1.0/(rmin-rheal_res))
                   +(2.0/(rheal_res*rheal_res*rheal_res))*log((rmin-rheal_res)/rmin);
